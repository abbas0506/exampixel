    ...
    <div class="container">
        ...
        <div class="grid grid-col-1 md:grid-cols-2 h-96 border mt-5">
            <div class="grid place-items-center bg-slate-200 p-8">
                <form action="{{ route('admin.mappings.store') }}" class="grid gap-3 md:w-3/4" method="post">
                    @csrf
                    ...
                    <select name="type_id" id="" class="custom-input">
                        ...
                        <option value="{{ $type->id }}" @if($type->id==session('typeId')) selected @endif>{{ $type->name }}</option>
                        @endforeach
                    </select>
                        ...
                        <tr class="tr">
                            ...
                            <td>
                                <div class="flex justify-center items-center space-x-2">
                                    ...
                                    <form action="{{route('admin.subtypes.destroy', $mapping->subtype)}}" method="POST" onsubmit="return confirmDel(event)">
                                        @csrf
                                        @method('DELETE')
