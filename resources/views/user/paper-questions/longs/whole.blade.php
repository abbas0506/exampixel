    ...
    <div class="container">
        ...
        <div class="flex flex-wrap items-center justify-between w-full mt-8 gap-y-4">
            <div class="flex items-center space-x-4 text-slate-600">
                ...
                <a href="{{ route('user.papers.partialQuestions.create', $paper) }}" class="tab">Partial Q. (has parts)</a>
            </div>
        </div>

        <form id='data-form' action="{{ route('user.papers.wholeQuestions.store', $paper) }}" method="post">
            @csrf
